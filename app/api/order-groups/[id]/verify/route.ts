...
// PATCH /api/order-groups/:id/verify -- panitia konfirmasi pembayaran sudah masuk.
// Setelah ini barcode ambil pesanan aktif untuk pembeli.
export async function PATCH(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (group.status !== "WAITING_VERIFICATION") {
    return NextResponse.json({ error: "Status tidak bisa dikonfirmasi dari sini" }, { status: 409 });
  }
